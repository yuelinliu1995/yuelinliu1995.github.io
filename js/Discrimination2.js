...
function saveFeedback(){
    if (subjectid.charAt(subjectid.length-1)<TotalParts){
      ...
      window.location.href = link[sequence[subjectid.charCodeAt(2)%TotalParts][subjtimes-1]] + '?'+subjectid+ subjtimes;
        ...
        db.collection('Discriminate-Short').doc(subjectid).update({
          comment:comment,
        })
...
function StarPra(){
    ...
    ntrial=0;
    ndown=0;
    timeBetweenSounds=[1.5,pracstandard+1.5,pracstandard+1.5+Pracdisduration[PracStiSeq[ntrial]]]
    playSoundPra();
}
function playSoundPra( ){
    ...
    for (var i = 0; i < Clicktimes; i++) {
      ...
      if (i==Clicktimes-1){
         source.stop(time + timeBetweenSounds[i] + (i+2)* buffer[0].duration);
          source.onended = function(){
            starttime = window.performance.now();
            changediv(speaker,Question);document.addEventListener('keydown',NextTrialSettingPra,true);
          }
      }
    }
  }
...
function NextTrialSettingPra(e){
  if (e.keyCode == 37|| e.keyCode == 39 ) {
    ...
    ntrial++
    timeBetweenSounds=[0.5,pracstandard+0.5,pracstandard+0.5+Pracdisduration[PracStiSeq[ntrial]]]
     // press other keys
     } else {
...
function StarExp(e){
    ...
    if (e.keyCode == 13){
      var elem2 = document.getElementById("myProgress");
        ...
        document.removeEventListener('keydown',StarExp,true);
    timeBetweenSounds=[1.5,standard+1.5,standard+1.5+disduration[StiSeq[ntrial]%levels]]
    playSound();
  }
}
function playSound( ){
    ...
    for (var i = 0; i < Clicktimes; i++) {
      ...
      if (i==Clicktimes-1){
         source.stop(time + timeBetweenSounds[i] + (i+2)* buffer[0].duration);
          source.onended = function(){
            starttime = window.performance.now();
            changediv(speaker,Question);document.addEventListener('keydown',NextTrialSetting,true);
...
function NextTrialSetting(e){
  if (e.keyCode == 37|| e.keyCode == 39 ) {
   ...
   console.log();
    ...
    // set next trial parameter
    ntrial++
    timeBetweenSounds=[0.5,standard+0.5,standard+0.5+disduration[StiSeq[ntrial]%levels]]
     // press other keys
     } else {
      alert("Please press ← or → arrow key");
            changediv(Question,Wrong)
...
function NextTrial(b){
    ...
    if (b>=TrialLim){
      ...
      // uploading data
      db.collection('Discriminate-Short').doc(subjectid).set({
        RTtime:RTtime,
        ScreenY:ScreenY,
        ScreenX:ScreenX,

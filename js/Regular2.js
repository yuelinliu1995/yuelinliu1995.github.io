...
function saveFeedback(){
    if (subjectid.charAt(subjectid.length-1)<TotalParts){
      ...
      window.location.href = link[sequence[subjectid.charCodeAt(2)%2][subjtimes-1]] + '?'+subjectid+ subjtimes;
        ...
        db.collection('Regular-Short').doc(subjectid).update({
          comment:comment,
        })
...
// presentation 7 points in total and 12 trials for each condition
...
function NextTrial(b){
    ...
    if (b>=TrialLim){
      ...
      // uploading data
      db.collection('Regular-Short').doc(subjectid).set({
        RTtime:RTtime,
        ScreenY:ScreenY,
        ScreenX:ScreenX,
